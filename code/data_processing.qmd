...
```
4. Daily total of each household
```{r}

      daily_avg <-
        colSums(aggregate(df, list(rep(
          1:(nrow(s04_0) %/% 3600 + 1),
          each = 1800,
          len = nrow(df)
        )), sum,
...
```
3. Compute semi-hourly consumption for each phase, kwh
```{r}
...
semihour_consum <- function(household) {
  for (phase in 0:3) {
      ...
      semihour_avg <-
        rowSums(aggregate(df, list(rep(
          1:(nrow(df) %/% 1800 + 1),
          each = 1800,
          len = nrow(df)
        )), sum,
          na.rm = TRUE,
          na.action = NULL)[-1]) / 3600000
      
      semihr_power <- data.frame(
        household = rep(household, 48),
        phase = rep(0:3, each=48),
        semihr = semihour_avg,
        hour = seq(0,23.5,0.5)
      )
...
hour_avg <- colSums(aggregate(s04_0, list(rep(1:(nrow(s04_0) %/% 1800 + 1), each = 1800, len = nrow(p04_01))), sum,
          na.rm = TRUE,
          na.action = NULL)[-1])/3600000
```